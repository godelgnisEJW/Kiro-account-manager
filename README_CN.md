...
- **新增**: 反代服务同步支持 GPT-5.6；`reasoning.effort: none` 可正确关闭思考模式
- **增强**: 完善 OpenAI Responses API 输入/输出转换，兼容 developer 消息、图文/文件输入、工具调用与工具结果
- **增强**: 支持通过 `additional_tools` 传入自定义函数工具，保留并正确转发工具调用结果
- **修复**: 跨多个流式分片泄漏的工具调用 XML 现在会被解析、去重并恢复为结构化工具调用，避免原始标签展示或 Agent 工具不执行

#### 📊 首页额度数据分析

- **新增**: 首页新增月度额度分析看板，展示总额度、已使用/剩余额度、日均消耗、月底预测、每日趋势及订阅类型分布
- **新增**: 自动保留当月账号额度快照，并提供额度区间分布和高消耗账号排行
- **新增**: 首页账号告警聚合，快速发现已封禁、7 天内到期及额度使用率 ≥90% 的账号；支持跳转账户列表并自动筛选

#### ✉️ 注册与 Token 刷新

- **新增**: 注册流程支持 GPTmail（`mail.chatgpt.org.uk`）验证码获取，兼容私有域名直收与 Cloudflare 转发模式
- **改进**: 主进程新增独立 Token 刷新调度器；即使窗口最小化或关闭，临近过期的账号 Token 仍会自动刷新
- **修复**: 批量注册的验证码并发处理、TLS Session 重建、浏览器认证信息解析与 Proton 配置校验问题

#### ⚡ 代理与账号管理稳定性

- **改进**: 配额耗尽账号按重置时间自动恢复可用；Token 刷新改为单飞执行，避免并发刷新和误切号
- **改进**: 流式响应支持背压处理，避免慢客户端导致内存堆积；提示词缓存键改为深度稳定序列化
- **修复**: 保留代理日志和缓存统计数据；修复 MITM 代理握手期间可能丢失请求体分片的问题
- **修复**: Windows 安装路径包含空格时的机器码提权重启失败；优化 Enterprise profileArn 获取与 OpenCode 1.15+ 兼容性
